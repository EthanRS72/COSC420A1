...
def smallnorb_classifier(load_from_file = False, verbose = True, reg_wdecay_beta = 0, reg_dropout_rate = 0, reg_batch_norm = False, data_aug=False):
    ...
    save_name = os.path.join('saved', 'smallnorbclassify2normxxxxxx_rwd%.1e_rdp%.1f_rbn%d_daug%d' % (reg_wdecay_beta,reg_dropout_rate,int(reg_batch_norm),int(data_aug)))
    ...
    if load_from_file and os.path.isfile(net_save_name):
      ...
      # ************************************************
        ...
        net.add(tf.keras.layers.Conv2D(filters = 96, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same", input_shape=(96, 96, 2)))
        net.add(tf.keras.layers.Conv2D(filters = 96, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        net.add(tf.keras.layers.Conv2D(filters = 96, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        if reg_batch_norm:
            # Batch norm layer 1
            net.add(tf.keras.layers.BatchNormalization())
        net.add(tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2)))
        net.add(tf.keras.layers.Conv2D(filters = 48, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        net.add(tf.keras.layers.Conv2D(filters = 48, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        net.add(tf.keras.layers.Conv2D(filters = 48, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        if reg_batch_norm:
            # Batch norm layer 1
            net.add(tf.keras.layers.BatchNormalization())
        net.add(tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2)))
        net.add(tf.keras.layers.Conv2D(filters = 24, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        net.add(tf.keras.layers.Conv2D(filters = 24, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        net.add(tf.keras.layers.Conv2D(filters = 24, kernel_size = (3, 3), strides = (1,1), activation='relu', padding = "same"))
        if reg_batch_norm:
            # Batch norm layer 1
